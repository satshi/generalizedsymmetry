...
\begin{proposition}[定理]
  Ising圏の対称性を持つ理論は$0$温度で「自明にギャップあり相」ではない。
\end{proposition}
...
次に同じ分配関数をBの方をEuclid時間と思って演算子形式で考えます。今度は空間内の１点にトポロジカル欠陥があるような系の十分低い温度での分配関数になります。この系のHamiltonianを$H_B$、エネルギー$0$の状態のHilbert空間を$V$とすると
\begin{align}
