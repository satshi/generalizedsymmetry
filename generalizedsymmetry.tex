...
\subsection{Kramers-Wannier双対性と、その証明}
...
\begin{align}
  \Zgauged(K)&=\frac{1}{2^{V}}\sum_{\{c\}}\prod_{\text{リンク}\ell}Z_{\ell},\label{KWaux1}\\
  Z_{\ell}&=\sum_{b=0,1}\exp(K(-1)^{b}+\pi i b(c_p+c_q))\quad (p,q \text{は$\ell$に接している２つのプラケット})
\end{align}
